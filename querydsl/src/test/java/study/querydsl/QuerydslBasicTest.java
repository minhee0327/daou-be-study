...
public class QuerydslBasicTest {
    ...
    public void startQueryDsl() {
        QMember m1 = new QMember("m1"); //alias 지정이 가능 (같은 테이블을 조인해야하는 경우에만 쓰면 됨 그렇지 않으면 사용할일이 거의 없습니다.)

        //결국 queryDSL -> JPQL
        Member findMember = queryFactory
                .select(m1)
                .from(m1)
                .where(m1.username.eq("member1"))  //sql injection 방지, prepareStatement 를사용, username 잘못 쓸 실수 방지, 파라미터 바인딩
                .fetchOne();
...
//                .fetchFirst();
    ...
    //tuple 실무에서 많이 사용되지는 않음 | 여러타입일때 사용 | +) DTO 로 더 많이 사용|
    @Test
...
//                .fetch();
    ...
    public void join_on_no_relation() {
        ...
        //기존에 LeftJoin(member.team, team) -> id 로 비교해서 같은 데이터를 가져온다.
        //아래 내용의 경우에는 member 와 team 을 join 할 때 member.username 과 team.name 을 비교해서 가져온다.
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
    ...
    //페치 조인 : SQL 에서 제공하는 기능은 아님
    //SQL 조인을 활용해서 연관된 엔티티를 한번에 조회하는 기능이다. 주로 성능 최적화에 사용된다.
    @Test
    public void fetchJoinNo() {
        //영속성 컨텍스트를 안비우면 결과를 제대로 보기 어려움.
        em.flush();
        em.clear();
        ...
        Member findMember = queryFactory
                ...
                .where(member.username.eq("member1"))
                .fetchOne();
        //Team -> Lazy Loading 이라서 team 은 조회되지 않음 위에 로직만 실행하면
        //증명: EntityMangerFactory 로 증명. isLoaded : 초기화 되었는지 유무를 확인할 수 있음.
        //Team 은 아직 초기화가 안됬으니 false (fetch -> Lazy Loading 상태)
    ...
    public void complexCase() {
        //요런 경우보단 db 에서는 최소한의 그루핑하는걸 더 추천한다...
        List<String> result = queryFactory
    ...
    //where 절은 null 인 경우 무시하게됨 (null 이 아닌경우만 체크)
    //반환타입
    ...
    public void bulkUpdate() {
        ...
        // 벌크 연산시 영속성 컨텍스트에 남은 내용과 db 에 반영된 내용이 다를 수 있다.
        // 따라서 벌크 연산후에는 flush && clear 를 해주는 습관..!
    ...
    public void sqlFunction() {
        List<String> result = queryFactory
                .select(
                        Expressions
                                .stringTemplate("function('replace', {0}, {1}, {2})", member.username, "member1", "M"))
                .from(member)
                .fetch();
